...
func (daemon *Daemon) CheckpointCreate(name string, config types.CheckpointCreateOptions) error {
	...
	err = daemon.containerd.CreateCheckpoint(context.Background(), container.ID, checkpointDir, config.ParentPath, config.CriuPageServer, config.Exit, config.OpenTcp, config.UnixSockets, config.Terminal, config.FileLocks)
	if err != nil {
